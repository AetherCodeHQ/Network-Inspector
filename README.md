...
> Network-Inspector - Open source tool by AetherCodeHQ

`cli` `golang` `inspection` `network` `security`

## What is it?

**Network Inspector** is a security tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Network-Inspector.git
cd Network-Inspector
go build -o Network-Inspector .
```

### Run
```bash
./Network-Inspector --help
```
...
```bash
# Basic usage
./Network-Inspector --path ./target

# With options
./Network-Inspector --path ./target --format json --output report.json

# Verbose mode
./Network-Inspector --path ./target --verbose
```
...
### Basic scan
```bash
./Network-Inspector --path ./my-project
```

### JSON report
```bash
./Network-Inspector --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Network Inspector
  run: ./Network-Inspector --path . --format json --output report.json
```
